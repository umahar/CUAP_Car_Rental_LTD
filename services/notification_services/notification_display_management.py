...
from data import prompts, menus
...
def display_notifications(user_id):
    ...
    opt = get_user_option(menus.main_notification_menu, prompts.STANDARD_MENU)
    if opt == 1:
    ...
    if opt == 3:
        category = get_user_option(
            menus.notification_categories, "Select a category for the notifications"
        )
        if category != 0:
            sort = get_user_option(
                sorting_menu, "Select your sorting for the notifications"
...
def display_unread_notifications(user_id, sort):
    ...
    if sort == 2:
        sort_by = "DESC"
    notifications = get_all_unread_notifications(user_id, sort_by)
    if notifications:
        print_notifications(notifications)
    else:
        print(prompts.NO_NOTIFICATIONS)
...
def display_notifications_by_category(user_id, sort, category):
    ...
    if sort == 2:
        sort_by = "DESC"
    notifications = get_notifications_by_category(user_id, category, sort_by)
    if notifications:
        print_notifications(notifications)
    else:
        print(prompts.NO_NOTIFICATIONS)
